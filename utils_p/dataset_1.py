...
class SceneDatasetCV(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return torch.tensor(X), torch.tensor(Y), torch.tensor(poses), self.video_ids[i], self.frames[i], \
               self.person_ids[i], horizontal_flip, torch.tensor(egomotions), self.scales[i], self.turn_mags[i], \
               self.scales_all[i]
