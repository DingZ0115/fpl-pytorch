...
COLORS = [[85.0, 0.0, 255.0], [0.0, 0.0, 255.0], [0.0, 85.0, 255.0], [0.0, 170.0, 255.0], [0.0, 255.0, 255.0],
          [0.0, 255.0, 170.0], [0.0, 255.0, 85.0], [0.0, 255.0, 0.0], [85.0, 255.0, 0.0], [170.0, 255.0, 0.0],
          [255.0, 255.0, 0.0], [255.0, 170.0, 0.0], [255.0, 85.0, 0.0], [255.0, 0.0, 0.0], [170.0, 0.0, 255.0],
          [255.0, 0.0, 170.0], [255.0, 0.0, 255.0], [255.0, 0.0, 85.0]]
...
def draw_line(img, traj, color, ratio, thickness=2, skip=1):
    for idx, (d1, d2) in enumerate(zip(traj[:-1], traj[1:])):
        if idx % skip != 0:
            continue
        cv2.line(img, (int(d1[0] * ratio), int(d1[1] * ratio)), (int(d2[0] * ratio), int(d2[1] * ratio)), color,
                 thickness)
    return img
...
def draw_pose(img, point, pose, scale, ratio):
    ...
    for idx, (x, y) in enumerate(pose):
        cv2.circle(canvas, (int((x * scale + cx) * ratio), int((y * scale + cy) * ratio)), int(scale / 7 * ratio),
                   COLORS[idx], -1)
...
def plot_trajectory_eval(save_dir, data_dir, w, h, past, ground_truth, vid, frame, pid, predicted, egomotion, pose,
                         pred_ego, scale, pred_pose, flipped=False):
    past_len = len(past)
...
def plot_trajectory_full(save_dir, data_dir, w, h, past, ground_truth, vid, frame, pid, predicted, egomotion, pose,
                         pred_ego, scale, pred_pose, flipped=False):
    past_len = len(past)
    pred_len = len(ground_truth)
...
def plot_prob_all(save_dir, w, h, mean, std, input_data, probs, predicted):
    ...
    plt.figure()
    plt.imshow(canvas, cmap=cmap, vmin=0, vmax=np.max(canvas), extent=[0, w, 0, h], aspect='auto',
